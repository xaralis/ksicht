...
zip_validator = validators.RegexValidator(r'^\d{3} ?\d{2}$', 'Zadejte PSČ ve formátu 123 45.')
phone_validator = validators.RegexValidator(r'^(\+420)? ?[1-9][0-9]{2} ?[0-9]{3} ?[0-9]{3}$', 'Zadejte telefon ve formátu +420 777 123123.')
...
class KsichtRegistrationForm(
    ...
    phone = forms.CharField(label="Telefon", max_length=20, required=False, validators=[phone_validator])
    street = forms.CharField(label="Ulice", max_length=100)
    city = forms.CharField(label="Obec", max_length=100)
    zip_code = forms.CharField(label="PSČ", max_length=10, required=True, validators=[zip_validator])
    ...
    def save(self, commit=True):
        ...
        user.participant = Participant(**py_.pick(cd, 'phone', 'street', 'city', 'zip_code', 'country', 'school', 'school_alt_name', 'schole_alt_street', 'school_alt_city', 'school_alt_zip_code'))
